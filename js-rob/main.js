...
var pgal = {
  ...
  PGObj: function (n) {
    ...
    this.anim = function () {
      ...
      if (f > 0) {
        var d = 1000 / f;
        var X = pgal.nw * 0.5 + (this.x - pgal.x - pgal.cx) * d;
        var Y = pgal.nh * 0.5 + (this.y - pgal.y - pgal.cy) * d;
        var W = d * this.w * pgal.zoom;
        var H = d * this.h * pgal.zoom;
        this.obs.left = to_px(X - W * 0.5);
        this.obs.top = to_px(Y - H * 0.5);
        this.obs.width = to_px(W);
        this.obs.height = to_px(H);
        this.oxs.visibility =
          this.CF-- > 0 && Math.random() > 0.9 ? "hidden" : "visible";
        this.oxs.left = to_px(X - W * 0.5);
        this.oxs.top = to_px(Y + H * 0.5);
        if (pgal.zt - pgal.z < 20) {
          if (!this.F) {
            ...
            for (var i = 0; i < tn; i++) {
              ...
              this.sto[i] = setTimeout(
                "pgal.O[".concat(n, '].oxt.innerHTML = "', T, '";'),
                Math.round(f / 4) + 10 * i
              );
            }
          }
...
document.onmousemove = function (e) {
  ...
  pgal.xm = (e.x || e.clientX) - pgal.nx - pgal.nw * 0.5;
  pgal.ym = (e.y || e.clientY) - pgal.ny - pgal.nh * 0.5;
};
